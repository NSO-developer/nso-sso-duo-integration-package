...
class XmlParser:
    ...
    def __init__(self, xml_string: str, certificate: Optional[X509], pkey_string: Optional[str]):
        ...
        self._logger.info("Starting parse in XmlParser")

        if certificate is not None:
            self.certificate = certificate
        
        if pkey_string is not None:
        ...
        if self.is_encrypted():
            self._logger.info("Encrypted response - Attempting decryption")
            self.xml_tree = self.decrypt_response(self.xml_tree, self.pkey_string, self.certificate)
        elif self.is_signed():
            self._logger.info("No encryption found - Attempting signature validation")
            self.xml_tree = self.parse_signed(self.xml_tree, self.certificate)
        self._logger.info("Parsed XML: "+lxml.etree.tostring(self.xml_tree,method='c14n',
                                   exclusive=True).decode('utf-8'))
    
    def decrypt_response(self, xml_tree: XmlNode, pkey_string: str, certificate: X509) -> XmlNode:
        ...
        #Response sig validation
        dec_sig = dec_xml_tree.xpath('/saml:Assertion/ds:Signature',
                                namespaces=self.get_namespace_map())
        if dec_sig:
            dec_xml_tree = XMLVerifier().verify(dec_sig, x509_cert=certificate).signed_xml
